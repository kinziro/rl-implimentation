...
from network import InferenceNet2, EmbeddingNet2, PolicyNet
...
class OnPolicyAlgorithm(BaseAlgorithm):
    ...
    def _setup_model(self) -> None:
        ...
        policy_observation_dim = self.observation_space.shape[0] + self.task_embedding_dim + self.env_embedding_dim
        ...
        self.rollout_buffer = EmbeddingRolloutBuffer(
            ...
            self.action_space,
            self.task_embedding_dim+self.env_embedding_dim,
            self.device,
            gamma=self.gamma,
        ...
        self.embedding_net = EmbeddingNet2(task_id_dim=self.task_id_dim, env_id_dim=self.env_id_dim, task_embedding_dim=self.task_embedding_dim, 
                                           env_embedding_dim=self.env_embedding_dim, normalization_coeff=None, device=self.device, **embedding_kwargs).to(self.device)
        ...
        self.inference_net = InferenceNet2(observation_space=self.observation_space.shape[0], action_space=self.action_space.shape[0], 
                                           embedding_dim=self.task_embedding_dim+self.env_embedding_dim, normalization_coeff=None, 
    ...
    def collect_rollouts(
        ...
        self.act_h_manager = ActionHistoryManager(n_envs=self.env.num_envs, n_history=self.n_obs_history-1)
        #self.act_h_manager_cut_before_z = ObservationHistoryManager(n_envs=self.env.num_envs, n_history=self.n_obs_history-1)
        #self.act_h_manager_cut_before_z.add([0.0, 0.0])
        ...
        t_id = th.tensor(np.array(self.env_task_id_list)[:, :self.task_id_dim]).float().to(self.device)
        e_id = th.tensor(np.array(self.env_task_id_list)[:, self.task_id_dim:]).float().to(self.device)
        z, embedding_log_prob, embedding_entropy = self.embedding_net.forward(t_id, e_id)
        ...
        while n_steps < n_rollout_steps:
            ...
            # 推論NNによる潜在変数を取得
            cut_obs_add_action_before_z = th.cat([th.tensor(obs_h), th.tensor(act_h), cut_action_before_z], dim=1)
            ...
            self.act_h_manager.add(clipped_actions)
            self.act_h_manager.reset(dones)
    ...
    def learn(
        ...
        while self.num_timesteps < total_timesteps:
            ...
            save_num = self.num_timesteps // 500000
            if save_num > self.last_save_num:
                import os
                fn = os.path.join(savedir, f'embedding_model_{self.num_timesteps}')
                self.save(fn)
...
class ActionHistory:
    def __init__(self, n_history: int):
        self.n_history = n_history
        self.obs_h = [[0, 0] for _ in range(n_history)]
        self.index = -1
        self.reset_flag = False
    ...
    def reset(self):
        self.obs_h = [[0, 0] for _ in range(self.n_history)]
        self.reset_flag = False
...
class ActionHistoryManager:
    def __init__(self, n_envs, n_history):
        self.n_envs = n_envs
        self.envs = [ActionHistory(n_history) for _ in range(n_envs)]
