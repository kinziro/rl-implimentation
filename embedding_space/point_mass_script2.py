...
savedir = '{}multi_task_2/'.format(base_savedir)
...
if config["learn"]["flag"]:
    ...
    for loop in range(2):
        for task_int, task_id in zip(task_int_list, task_id_list):
            print(f'--- loop:{loop}  task:{task_int} ---')
            learn_savedir = os.path.join(savedir, f'{loop}_{task_int}')
            os.makedirs(learn_savedir, exist_ok=True)
            set_task_id_list = [task_id for _ in range(n_cpu)]
            model.reset_task_ids(set_task_id_list)
            model.learn(total_timesteps=total_timesteps)

            plot_history(model.policy_loss_history, title='policy_loss', xlabel='episode', ylabel='loss', figdir=learn_savedir)
            plot_history(model.inference_loss_history, title='inference_loss', xlabel='episode', ylabel='loss', figdir=learn_savedir)
            plot_history(model.embedding_loss_history, title='embedding_loss', xlabel='episode', ylabel='loss', figdir=learn_savedir)
            plot_history(model.reward_history, title='reward_sum', xlabel='episode', ylabel='reward_sum', figdir=learn_savedir)
            plot_history(model.R_history, title='R_sum', xlabel='episode', ylabel='R_sum', figdir=learn_savedir)
            error_zs = np.array(model.error_zs_history)
            for col in range(error_zs.shape[1]):
                plot_history(error_zs[:, col], title=f'error_z_{col}', xlabel='episode', ylabel='error_z', figdir=learn_savedir)
